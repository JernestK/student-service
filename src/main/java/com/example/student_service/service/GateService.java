...
public class GateService {
    ...
    public byte[] generatePdfFile(List<Student> students) throws IOException {
        try (PDDocument document = new PDDocument();
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
                ...
                contentStream.showText("Liste des ID étudiants portail captif - SJI");
                ...
                float rowHeight = 20;
                float cellMargin = 5;
                String[] headers = {"N°", "Nom", "Prénom", "Matricule", "Mot de passe"};
                ...
                columnWidths[1] = tableWidth * 0.30f;
                columnWidths[2] = tableWidth * 0.30f;
                columnWidths[3] = tableWidth * 0.20f;
                columnWidths[4] = tableWidth * 0.15f;
    ...
    private void drawTable(PDPageContentStream contentStream, float y, float margin, float width, float rowHeight, int rows, int cols) throws IOException {
        ...
        columnWidths[1] = width * 0.30f; // 20% pour la 2e colonne
        columnWidths[2] = width * 0.30f; // 5% pour la 3e colonne
        columnWidths[3] = width * 0.20f; // 20% pour la 4e colonne
        columnWidths[4] = width * 0.15f; // 20% pour la 5e colonne
    ...
    private byte[] generateExcelFile(List<Student> students) throws IOException {
        ...
        Sheet sheet = workbook.createSheet("GateEtudiants");
        Row headerRow = sheet.createRow(0);
        headerRow.createCell(0).setCellValue("Nom");
        headerRow.createCell(1).setCellValue("Prénom");
        headerRow.createCell(2).setCellValue("Identifiant Portail");
