...
import com.example.student_service.service.GateService;
...
public class GateController {
    ...
    public String handleFileUpload(@RequestParam(value = "type", required = false) String type,
                                   ...
                                   HttpSession session) {
        try {
            List<Student> students = gateService.importStudentsFromExcel(file.getInputStream());
    ...
    public ResponseEntity<byte[]> downloadFile(@PathVariable String format,
        ...
        try {
            byte[] fileContent = gateService.generateExportFile(students, format);
            HttpHeaders headers = new HttpHeaders();
            String filename = type + "_students." + (format.equalsIgnoreCase("pdf") ? "pdf" : "xlsx");
